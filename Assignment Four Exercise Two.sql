...
select orders.order_id, customers.customer_name, orders.order_date, orders.total_amount, sum(order_items.quantity) as total_quantity
from orders
join customers on orders.customer_id = customers.customer_id
join order_items on orders.order_id = order_items.order_id
group by orders.order_id, customers.customer_name, orders.order_date, orders.total_amount;

create view product_sales as 
select products.product_id, products.product_name, categories.category_name, sum(order_items.unit_price * order_items.quantity) as total_revenue
from products
